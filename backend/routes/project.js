...
router.post('/assignProject', function(req, res, next) {
  var sql = 'update project set current_department = ?, status = ? where project_id = ?';
  connection.query(sql, [req.body.department, req.body.status, req.body.projectId], function(err, result) {
    if(err) {
      console.log(err.sqlMessage);
      res.status(500).json({
