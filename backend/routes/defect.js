    ...
    if (err) {
      ...
      iterationNo(req.params.projectId, function(err, iteration_no){
        if(err) {
          ...
          const table = "defects_" + req.params.projectId;
          const date = new Date();
          const data = [
            [
              iteration_no,
...
function createDefectTable(projectId, callback) {
  const sql = "create table if not exists ??(iteration_no int, defect_id int auto_increment primary key, defect_name varchar(50) not null, defect_category varchar(50) not null, defect_type varchar(50) not null, status varchar(20) not null, description varchar(200) not null, log_data varchar(1000), assigned_department varchar(20), assigned_user int, date date, assign_status int not null, foreign key(iteration_no) references ??(iteration_no), foreign key(defect_category) references defect_category(category_name), foreign key(defect_type) references defect_type(type_name),  foreign key(status) references defect_status(status),  foreign key(assigned_department) references department(department_id), foreign key(assigned_user) references user(user_id))";
  const table = "defects_" + projectId;
  const table1 = "pro_" + projectId;
...
router.get('/getUserDefects/:projectId/:userId', (req, res, next) => {
  const sql = 'select * from ?? where assigned_user = ?';
  const table = 'defects_' + req.params.projectId;
  connection.query(sql, [table, req.params.userId], function(err, result) {
    if(err) {
      console.log(err.sqlMessage);
      res.status(500).json({
        message: err.sqlMessage
...
router.post('/assignDefect', (req, res, next) => {
  const sql = "update set assigned_department = ?, assigned_user = ?, assign_status = 1" + " where defect_id = ?";
  const table = "defects_" + req.body.projectId;
  connection.query(sql, [table, req.body.departmentId, req.body.userId, req.body.defectId], function(err, result) {
    if(err) {
      console.log(err.sqlMessage);
      res.status(500).json({
        message: err.sqlMessage
...
router.post('/bulkDefects', extractFile, (req, res, next) => {
  ...
  iterationNo(req.body.projectId, function(err, iteration_no){
    if(err) {
      ...
      const sql = "LOAD DATA INFILE ? into table ?? FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n' IGNORE 1 ROWS (defect_name,	defect_category,	defect_type,	status,	description,	log_data) SET date = CURDATE(),	assign_status = 0, iteration_no = ?";
      const table = "defects_" + req.body.projectId;
      connection.query(sql, [path, table, iteration_no], function(err, result) {
        if(err) {
          res.status(500).json({
...
router.put('/closeDefect', (req, res, next) => {
  const sql = "update ?? set status = 'closed' where defect_id = ?";
  const table = "defects_" + req.body.projectId;
  connection.query(sql, [table, req.body.defectId], function(err, result) {
    if(err) {
      console.log(err.sqlMessage);
      res.status(500).json({
...
router.delete('/deleteDefect/:projectId/:defectId', (req, res, next) => {
  const sql = "delete from ?? where defect_id = ?";
  const table = "defects_" + req.params.projectId;
  connection.query(sql, [table, req.params.defectId], function(err, result) {
    if(err) {
      console.log(err.sqlMessage);
      res.status(500).json({
